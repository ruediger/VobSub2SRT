...
* Building
You need tesseract and libavutil (part of the [[http://ffmpeg.org/][FFmpeg]] project). You also need cmake and a gcc to build it. With Ubuntu 12.10 you can install the dependencies with

#+BEGIN_EXAMPLE
  sudo apt-get install libavutil-dev libtiff5-dev libtesseract-dev tesseract-ocr-eng build-essential cmake pkg-config
#+END_EXAMPLE
